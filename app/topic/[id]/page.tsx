...
export default function TopicDetail() {
    ...
    const handleVote = async (choiceId: string) => {
        ...
        try {
            ...
            if (voteChoice && voteChoice !== choiceId) batch.update(topicRef, { [`votes.${voteChoice}`]: increment(-1), [`votes.${choiceId}`]: increment(1) });
            else batch.update(topicRef, { [`votes.${choiceId}`]: increment(1) });
            await batch.commit();
    ...
    const getSmartSortedList = (list: any[]) => {
        const valid = list.filter(r => !r.isHidden);
    ...
    const renderReasonCard = (r: any, idx: number) => {
        ...
        return (
            <div key={r.id || idx} className={`p-3 border rounded-xl bg-white shadow-sm mb-3 ${isTop3 ? 'border-yellow-400 ring-4 ring-yellow-50 bg-yellow-50' : ''}`}>
                {isTop3 && <div className="text-[10px] font-black text-yellow-600 mb-1 flex items-center gap-1">👑 第{idx + 1}位</div>}
                <div className={`text-xs font-bold ${textColor} mb-1`}>{option?.text} 派の意見</div>
                ...
                <div className="flex justify-between items-center border-t pt-2 border-gray-100">
                    ...
                    {userId !== r.userId && !reportedIds.has(r.id) && (
                        <button onClick={() => handleReportReason(r.id, r.userId)} className="text-[10px] text-gray-300 hover:text-red-500">通報</button>
                    )}
                </div>
            </div>
        );
    ...
    // ★コンテンツ閲覧権限の判定
    // 「投票済み」または「アーカイブ」または「議論/公開フェーズ」なら見れる
    // 言い換えると：「投票フェーズ」かつ「未投票」の場合は見れない
    const canViewContent = hasVoted || isArchive || currentPhase === 'discussion' || currentPhase === 'public';
        ...
        return (
            <div className="flex justify-center items-center min-h-screen bg-gray-50">
                <div className="bg-white p-10 rounded-2xl shadow-xl text-center max-w-lg border-2 border-orange-200 mx-4">
                    ...
                    <h1 className="text-2xl font-black text-gray-800 mb-4">公開までお待ちください</h1>
                    <p className="text-gray-500 mb-6">
                        このお題はまだ開始されていません。<br />
                        公開予定時刻までしばらくお待ちください。
                    </p>
                    <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 inline-block text-left">
                        <p className="text-xs font-bold text-orange-400 mb-1">タイトル</p>
                        <p className="font-bold text-gray-800 text-lg mb-2">{topicData.title}</p>
                        <p className="text-xs font-bold text-orange-400 mb-1">開始予定</p>
                        <p className="font-mono font-bold text-orange-600 text-xl">
                            {new Date(topicData.startDate).toLocaleString()}
                        </p>
                    </div>
                    <div className="mt-8">
                        <button onClick={() => router.push("/")} className="px-6 py-2 bg-gray-200 hover:bg-gray-300 rounded-lg font-bold text-gray-600 transition">
                            トップへ戻る
                        </button>
                    </div>
                </div>
    ...
    // 2. 公開前ロック処理（今回追加する部分）
    if (topicData.startDate && new Date(topicData.startDate) > now && topicData.status !== 'archived' && topicData.type !== 'archive') {
        return (
            <div className="flex justify-center items-center min-h-screen bg-gray-50">
                <div className="bg-white p-10 rounded-2xl shadow-xl text-center max-w-lg border-2 border-orange-200 mx-4">
                    <div className="text-6xl mb-6">⏳</div>
        ...
        <div className="flex justify-center bg-gray-50 min-h-screen">
            ...
            <main className="w-full max-w-4xl p-6 bg-white shadow-sm">
                ...
                <section className="mb-12">
                    <h2 className="text-xl font-bold mb-4">🗳️ 投票</h2>
                    {!requireReason && (
                        ...
                        <div className="mt-6 p-4 bg-orange-50 border-2 border-orange-200 rounded-xl">
                            ...
                            {tempReVoteChoice && (
                                <div className="relative">
                                    <textarea value={reason} onChange={e => setReason(e.target.value)} className="w-full p-3 border rounded-lg mb-1" maxLength={MAX_REASON_LENGTH} placeholder="理由を入力..." />
                                    <span className="absolute bottom-2 right-2 text-xs text-gray-400">{reason.length}/{MAX_REASON_LENGTH}</span>
                                </div>
                            )}
                            <div className="flex gap-2 mt-2">
                                <button onClick={submitReason} disabled={!tempReVoteChoice || isSubmitting} className="px-4 py-2 bg-orange-600 text-white font-bold rounded-lg shadow disabled:opacity-50">
                                    {isSubmitting ? '送信中...' : '確定'}
                                </button>
                ...
                {!canViewContent ? (
                    <div className="py-12 bg-gray-100 rounded-2xl border-2 border-dashed border-gray-300 text-center animate-in fade-in">
                        ...
                        <h3 className="text-xl font-bold text-gray-700 mb-2">投票すると閲覧できます</h3>
                    ...
                    <>
                        ...
                        <section className="mb-12">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold">💬 みんなの理由</h2>
                                {/* 並び替えボタン (折りたたみ部分に適用) */}
                                <div className="flex gap-2">
                                    <button onClick={() => setSortBy('likes')} className={`text-xs px-2 py-1 rounded border ${sortBy === 'likes' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>いいね順</button>
                                    <button onClick={() => setSortBy('newest')} className={`text-xs px-2 py-1 rounded border ${sortBy === 'newest' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>新着順</button>
                                </div>
                            </div>
                            ...
                            {!isArchive && hasVoted && isVotingPhase && !requireReason && (
                                <div className="mb-8 p-4 bg-blue-50 border border-blue-100 rounded-xl">
                                    ...
                                    <button onClick={submitReason} disabled={isSubmitting} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg shadow-md hover:bg-blue-700 transition disabled:opacity-50">
                            ...
                            <div className="space-y-6">
                                        ...
                                        {topicData.options?.map((opt, i) => {
                                            ...
                                            return (
                                                <div key={opt.id} className="bg-gray-50 p-3 rounded-2xl border">
                                                    <h3 className={`font-bold text-center mb-3 pb-2 border-b ${OPTION_TEXT_COLORS[i % 5]}`}>{opt.text} 派の主張</h3>
                                                    {displayList.length > 0 ? (
                                                        displayList.map((r, idx) => renderReasonCard(r, idx))
                                                    ) : <p className="text-xs text-gray-400 text-center py-4">まだ投稿がありません</p>}
                                                    {!isReasonsExpanded && smartList.length > 3 && (
                                                        <p className="text-center text-xs text-gray-400 mt-2">他 {smartList.length - 3} 件</p>
                                                    )}
                                                </div>
                                            );
                                        })}
                                ...
                                {!isDiscussionPhase && !isArchive && voteChoice && (
                                    <div>
                                        <h3 className="font-bold text-gray-700 mb-3">あなたと同じ {getOption(voteChoice)?.text} 派の意見</h3>
                                        {(() => {
                                ...
                                <div className="text-center mt-4">
                                    <button onClick={() => setIsReasonsExpanded(!isReasonsExpanded)} className="bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold py-2 px-6 rounded-full text-sm transition">
                        ...
                        <section className="pb-20">
                            <h2 className="text-xl font-bold mb-4">{isDiscussionPhase || isArchive ? "🗣️ 議論掲示板" : "🔒 陣営別掲示板"}</h2>

                            {isArchive && (
                                <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
                                    <button onClick={() => { setActiveBoardTab('discussion'); setCommentPage(1); }} className={`px-3 py-1 rounded-full text-sm font-bold ${activeBoardTab === 'discussion' ? COLORS.TAB_ACTIVE : COLORS.TAB_INACTIVE}`}>議論全体</button>
                                    {topicData.options?.map(opt => (
                                        <button key={opt.id} onClick={() => { setActiveBoardTab(opt.id); setCommentPage(1); }} className={`px-3 py-1 rounded-full text-sm font-bold ${activeBoardTab === opt.id ? COLORS.TAB_ACTIVE : COLORS.TAB_INACTIVE}`}>{opt.text}派</button>
                                    ))}
                                </div>
                            )}
                            ...
                            <div className="space-y-2 mb-6 max-h-[800px] overflow-y-auto pr-2">
                                {displayedComments.map((c, i) => (
                                    <div key={(c as any).id || i} className="p-3 bg-gray-50 border rounded-lg hover:bg-white transition">
                                        ...
                                        {c.quoteNumber && <div className="text-[10px] bg-gray-200 border-l-4 border-gray-400 pl-2 py-1 mb-2 text-gray-600 italic">No.{c.quoteNumber} への返信</div>}
                            ...
                            {totalPages > 1 && (
                                <div className="flex justify-center gap-4 text-sm mt-4 mb-4 items-center bg-gray-100 p-2 rounded-lg">
                                    <button disabled={commentPage === 1} onClick={() => setCommentPage(p => p - 1)} className="px-3 py-1 bg-white border rounded shadow disabled:opacity-30 font-bold">前へ</button>
                                    <span className="font-bold text-gray-700">{commentPage} / {totalPages} ページ</span>
                                    <button disabled={commentPage === totalPages} onClick={() => setCommentPage(p => p + 1)} className="px-3 py-1 bg-white border rounded shadow disabled:opacity-30 font-bold">次へ</button>
                                </div>
                            )}
                            ...
                            {!isArchive && hasVoted && (
                                <div className="p-4 bg-white border-2 border-gray-200 rounded-2xl shadow-lg">
                                    ...
                                    <button onClick={handleComment} disabled={isSubmitting} className="w-full py-3 bg-green-600 text-white font-black rounded-xl hover:bg-green-700 transition shadow-lg disabled:opacity-50">
